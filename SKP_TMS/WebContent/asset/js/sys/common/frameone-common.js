...
function checkRequired(obj)
{
	...
	if($objInput.hasClass("number")){
		...
		if( $objInput.val() == '' || parseFloat($objInput.val().trim()) == 0) {
			showMessage("ERR_REQUIRED", $objInput.attr("title") );
			$objInput.focus();
			return false;
		}else {
		    return true;
...
function checkMaxbyte(obj, maxByte)
{
	...
	if(val.getByte() > maxByte) {
		showMessage("ERR_MAXBYTE", maxByte);
		$objInput.select();
		$objInput.focus();
		return false;
	}else {
...
function validateRadioRequired(radioName, radioTitle){
	...
	if($chkRadio.val() === undefined){ 
		showMessage('ERR_REQUIRED', radioTitle);
...
function isShorter(o, len, noAlert){
	...
	if( ! noAlert && ret){
		showMessage('ERR_MINLEN', String(len)); //len 글자 이상 입력해 주십시오.
	}
	return ret;
}
