					...
					if(cv[0] === true) {
						...
						if ($t.p.cellsubmit === 'clientArray') {
							...
							*/
							...
							/** 차량번호가 지워져서 조건을 줌 */
							var fistChildrenName = $(cc).children().get(0).tagName;
							var isEqpNo = ($(cc).attr("aria-describedby").indexOf("EQP_NO") != -1);
							
							// 차량번호 셀이고, 첫번째 자식 태그가 input 
							if ( isEqpNo && fistChildrenName == "INPUT"  ) {
								$(cc).empty();	
								$($t).jqGrid("setCell",$t.rows[iRow].id,iCol, v2, false, false, true);
								$(cc).addClass("dirty-cell");

							// 차량번호 셀이 아닐때	
							} else if ( isEqpNo == false ) {
								$(cc).empty();	
								$($t).jqGrid("setCell",$t.rows[iRow].id,iCol, v2, false, false, true);
								$(cc).addClass("dirty-cell");
							}
